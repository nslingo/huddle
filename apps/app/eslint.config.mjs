...
export default antfu(
  {
    ...
    ignores: [
      ...
      'docs/',
      'cli/',
      // Generated type-reference shims — regenerated by their tooling, not hand-edited, and not
      // ours to lint. They must stay in tsconfig (they augment nativewind/uniwind types), so this
      // ignore is lint-only.
      'expo-env.d.ts',
      'nativewind-env.d.ts',
      'uniwind-types.d.ts',
  ...
  // Function/param metrics are JS-only core rules: they read a JavaScript AST (e.g.
  // `sourceCode.getAllComments()`). antfu lints Markdown by default, so leaving these in the
  // unscoped block below loaded them against a `.md` file's Markdown sourceCode and crashed the
  // whole run. Scope them to code files (this glob still covers the JS/TS code blocks antfu
  // extracts from Markdown, whose virtual paths end in .ts/.js — only the .md container is spared).
  {
